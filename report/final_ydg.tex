...
变为轮廓清晰的实体，图~\ref{fig:multiclass}展示了修复后五个类别的生成样本。

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{multiclass_gen.png}
    \caption{多类别条件生成样本（Q4 模型，从左到右：airplane / car / chair /
    rifle / table，先验 $z$ 采样，temperature=0，$\ell=0.01$）。car、chair、rifle
    轮廓清晰；airplane 呈现``类别平均''式的三角翼团块、table 有过度填充倾向，
    对应第~6 节讨论的独立边缘判决的模式平均问题。}
